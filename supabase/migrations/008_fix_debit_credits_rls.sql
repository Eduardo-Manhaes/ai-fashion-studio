-- ============================================================
-- 008 — Fix DEFINITIVO: Desabilitar RLS dentro da função RPC
-- ============================================================

-- Problema: As policies com auth.jwt() não funcionam em SECURITY DEFINER
-- Solução: Desabilitar RLS localmente dentro da função (seguro pois já valida user_id)
...
BEGIN
  ...
  -- Seguro porque a função já recebe e valida p_user_id
...
-- Recria refund_credits também com SET LOCAL
...
-- Comentário: SET LOCAL row_security = off é seguro porque:
-- 1. Só afeta a transação atual (não persiste)
-- 2. A função já valida p_user_id
-- 3. A função tem SECURITY DEFINER (já roda com privilégios elevados)
-- 4. Todas as queries internas usam WHERE user_id = p_user_id
